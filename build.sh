...
BLEED_HORIZONTAL="${BLEED_HORIZONTAL:-0.075in}"
...
BLEED_VERTICAL_PT=$(to_points "$BLEED_VERTICAL")
BLEED_HORIZONTAL_PT=$(to_points "$BLEED_HORIZONTAL")

PAPER_WIDTH_PT=$(echo "$TRIM_WIDTH_PT + 2 * $BLEED_HORIZONTAL_PT" | bc)
PAPER_HEIGHT_PT=$(echo "$TRIM_HEIGHT_PT + 2 * $BLEED_VERTICAL_PT" | bc)
...
MARGIN_TOP_ADJUSTED_PT=$(echo "$MARGIN_TOP_PT + $BLEED_VERTICAL_PT" | bc)
MARGIN_BOTTOM_ADJUSTED_PT=$(echo "$MARGIN_BOTTOM_PT + $BLEED_VERTICAL_PT" | bc)
MARGIN_INNER_ADJUSTED_PT=$(echo "$MARGIN_INNER_PT + $BLEED_HORIZONTAL_PT" | bc)
MARGIN_OUTER_ADJUSTED_PT=$(echo "$MARGIN_OUTER_PT + $BLEED_HORIZONTAL_PT" | bc)
