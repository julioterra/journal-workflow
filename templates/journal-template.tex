...
% Emoji support (limited - some emojis may not render)
% Note: For full emoji support, install Symbola or Noto Color Emoji font
% Using available system fonts as fallback
\IfFontExistsTF{Symbola}{
  \newfontfamily{\emojifont}{Symbola}[Scale=1.0]
}{
  \IfFontExistsTF{Noto Color Emoji}{
    \newfontfamily{\emojifont}{Noto Color Emoji}[Scale=1.0,Renderer=Harfbuzz]
  }{
    % Fallback to Apple Symbols (has limited emoji support)
    \newfontfamily{\emojifont}{Apple Symbols}[Scale=1.0]
  }
}
% Manual emoji command for wrapping emojis if needed
...
% Ensure index starts on a fresh spread (right page with blank left page)
\clearpage
\ifodd\value{page}
  % On odd (right) page: add blank even, then blank odd to start fresh spread
  \thispagestyle{empty}\hbox{}\clearpage\thispagestyle{empty}\hbox{}\clearpage
\else
  % On even (left) page: add blank odd, then blank even, then odd for index
  \thispagestyle{empty}\hbox{}\clearpage\thispagestyle{empty}\hbox{}\clearpage\thispagestyle{empty}\hbox{}\clearpage
\fi
