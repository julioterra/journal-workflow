...
\usepackage[
  ...
  top=$if(margintop)$$margintop$$else$0.75in$endif$,
  bottom=$if(marginbottom)$$marginbottom$$else$0.75in$endif$,
  inner=$if(margininner)$$margininner$$else$0.875in$endif$,
  outer=$if(marginouter)$$marginouter$$else$0.625in$endif$,
  bindingoffset=$if(marginbinding)$$marginbinding$$else$0.25in$endif$
]{geometry}
...
% BLOCKQUOTE STYLING
% ============================================================================
% Styled with background color and left border
\usepackage{changepage}
\usepackage{mdframed}
\definecolor{quotebg}{RGB}{245,245,250}
\definecolor{quoteborder}{RGB}{80,120,160}
...
\usepackage{rotating}  % For rotating landscape images
...
% Leave space for captions (70% of page height to ensure room for sideways captions)
...
% Set PDF color space to sRGB for print
...
\newcommand{\tag}[2]{%
  ...
  \@ifundefined{color@tag-bg-#2}{%
    ...
    \colorbox{tag-bg-default}{\strut\kern3pt\smash{#1}\kern3pt}%
  }{%
    % Use tag-specific background color
    \colorbox{tag-bg-#2}{\strut\kern3pt\smash{#1}\kern3pt}%
  }%
  \index[tags]{#1}%
  \xspace\hspace{0.5pt}%
}
...
% Front Matter
\frontmatter
\pagestyle{empty}  % Remove headers from front matter pages
...
% Author - 16pt unbolded
$if(author)$
{\fontsize{16pt}{20pt}\selectfont $author$\par}
\vspace{1em}
$endif$

% Title - 30pt bold
{\fontsize{30pt}{36pt}\selectfont\bfseries $title$\par}
\vspace{-0.5em}
...
% Year - 26pt bold
$if(year)$
{\fontsize{26pt}{31pt}\selectfont\bfseries $year$\par}
\vspace{0.5em}
$endif$

% Month Range - 10pt regular
$if(start_month)$
$if(end_month)$
{\fontsize{10pt}{12pt}\selectfont $start_month$ - $end_month$\par}
$else$
{\fontsize{10pt}{12pt}\selectfont $start_month$\par}
$endif$
\vspace{4em}
$endif$
...
% Add one blank page at the end for easy copy/paste padding
\clearpage
\thispagestyle{empty}
\mbox{}
